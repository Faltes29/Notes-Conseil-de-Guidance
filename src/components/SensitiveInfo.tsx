  ...
  return (
    <Card className="border-none shadow-lg bg-rose-50/30 backdrop-blur-sm border-l-4 border-l-rose-500">
      ...
      <CardContent className="space-y-6">
        ...
        {/* Rendez-vous personnes ressources */}
        <div className="space-y-4">
          ...
          {[1, 2].map((row) => (
            <div key={row} className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-xl bg-white border border-rose-100 items-end">
              <div className="space-y-2">
                <Label className="text-xs font-bold text-rose-500 uppercase tracking-wider">Personne ressource {row}</Label>
                <Select>
                  ...
                  <SelectContent className="rounded-xl">
                    {resourcePersons.map((person) => (
                      <SelectItem key={person} value={person.toLowerCase()}>{person}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
